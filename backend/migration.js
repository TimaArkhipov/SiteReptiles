...
export async function runMigrations() {
  ...
  // queryInterface.createTable('_people', {
  ...
  for (const file of list) {
    ...
    console.log('filePath: ' + path.join(migrationsPath, file))
    // import("/module-name.js")
    // const { up, down } = require(path.join(migrationsPath, file));
    const { up, down } = await import(path.join(migrationsPath, file)).then((module) => module.default);
